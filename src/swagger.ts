...
export const swaggerDocument: OpenAPIV3.Document = {
  ...
  info: {
    ...
    version: "1.9.0",
    description:
      "API livraison moto - Dakar et Saint-Louis (gestion commandes, clients, livreurs, paiements, blocages, navigation Mapbox). ✅ v1.9.0 : ajout de l'espace Client (inscription, commande avec assignation automatique, suivi temps réel, annulation).",
  },
  ...
  paths: {
    ...
    "/auth/password-reset/request": {
      post: {
        ...
        description:
          "Envoie un code à 6 chiffres par **email** à l'adresse associée au compte. L'identifiant peut être le **téléphone** ou l'**email**, pour un compte Admin, Livreur ou Client. Le code est valable **15 minutes**.",
        requestBody: {
          required: true,
          content: {
            "application/json": {
    ...
    "/client/register": {
      post: {
        ...
        description:
          "Crée un compte `User` avec le rôle `CLIENT` et un profil `Client` associé. L'email est **facultatif** — sans email, le client ne pourra pas utiliser la réinitialisation de mot de passe par code.",
        requestBody: {
          required: true,
          content: {
    ...
    "/client/me/localisation": {
      post: {
        ...
        description:
          "Enregistre la position GPS actuelle du client. Utilisée en priorité comme point de départ lors de la création d'une commande, pour trouver le livreur disponible le plus proche.",
        requestBody: {
          required: true,
          content: {
            "application/json": {
    ...
    "/client/commandes": {
      post: {
        ...
        description:
          "Crée une commande, calcule le tarif (distance réelle via Mapbox), puis cherche automatiquement le **livreur disponible le plus proche** de la position de départ et lui envoie une notification push. La commande apparaît immédiatement dans l'app du livreur (même flux que l'assignation admin).",
        requestBody: {
          required: true,
          content: {
    ...
    "/client/commandes/{commandeId}": {
      get: {
        ...
        description:
          "Retourne le statut de la commande et, si un livreur est assigné, sa position GPS actuelle (`livreur.lat` / `livreur.lng`) — à interroger par polling (ex: toutes les 5 secondes) pour afficher le livreur en mouvement sur la carte.",
        parameters: [
    ...
    "/admin/livreurs/{livreurId}/documents": {
      post: {
        ...
        description:
          "Upload les documents vers **Cloudinary**. La CNI recto + verso est **obligatoire** lors du premier upload. Les autres documents sont optionnels. Les fichiers acceptés sont : JPEG, PNG, WEBP, PDF (max 5 MB par fichier).",
        parameters: [
          {
            name: "livreurId",
            in: "path",
