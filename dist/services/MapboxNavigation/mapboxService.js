...
exports.calculerDistanceHaversine = exports.geocoderAdresse = exports.getProchaineInstruction = exports.recalculerRoute = exports.calculerRoute = void 0;
 ...
 */
const calculerRoute = async (points, profil = "driving-traffic") => {
    if (points.length < 2) {
        throw new Error("Au moins 2 points sont nécessaires pour calculer une route");
    }
    // Format Mapbox : "lng,lat;lng,lat;..."
    const coordStr = points.map((p) => `${p.lng},${p.lat}`).join(";");
    const { data } = await axios_1.default.get(`${MAPBOX_BASE}/directions/v5/mapbox/${profil}/${coordStr}`, {
        params: {
    ...
    const congestionsDetectees = route.legs.some((leg) => leg.annotation?.congestion?.some((c) => c === "heavy" || c === "severe"));
    // ── ETA ──
 ...
 */
const recalculerRoute = async (positionActuelle, destination, seuilDeviationMetres = 50 // Recalcul si déviation > 50m
) => {
    const distanceDeviation = await calculerDistanceVersRoute(positionActuelle, destination);
...
const geocoderAdresse = async (adresse) => {
    const { data } = await axios_1.default.get(`${MAPBOX_BASE}/geocoding/v5/mapbox.places/${encodeURIComponent(adresse)}.json`, {
        params: {
            access_token: MAPBOX_TOKEN,
            limit: 1,
            language: "fr",
...
const calculerDistanceVersRoute = async (position, destination) => {
    return (0, exports.calculerDistanceHaversine)(position, destination);
};
